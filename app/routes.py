...
def login():
    ...
    if form.validate_on_submit():
        ...
        ActivityLog.log_event(user.id, f"Login {user}", user.username)
        return redirect(url_for("index"))
...
@app.route("/logout")
def logout():
    ActivityLog.log_event(current_user.id, f"Logout {current_user}", current_user.username)
    logout_user()
    return redirect(url_for("index"))
...
def create_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        ActivityLog.log_event(current_user.id, f"Create: {post}", current_user.username)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        ActivityLog.log_event(user.id, "Register", user.username)
...
def up_vote(post_id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user.id, f"Up Vote: {post}", current_user.username)
...
def down_vote(post_id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user.id, f"Down Vote: {post}", current_user.username)
        return redirect(next_page or url_for("index"))
    else:
        return redirect(url_for("login"))
...
def up_vote_comment(comment_id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user.id, f"Up Vote: {comment}", current_user.username)
        return redirect(next_page or url_for("index"))
    else:
        return redirect(url_for("login"))
...
def down_vote_comment(comment_id):
    ...
    if current_user.is_authenticated:
        ...
        ActivityLog.log_event(current_user.id, f"Down Vote: {comment}", current_user.username)
        return redirect(next_page or url_for("index"))
    else:
        return redirect(url_for("login"))
